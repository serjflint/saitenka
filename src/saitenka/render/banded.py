...
The PIL half of the banded engine (:mod:`saitenka.render.window` stays PIL-free so it can be
...
one-shot :func:`saitenka.panel.compose_panel` stack of the same blocks (proven in
...
from saitenka.render.window import OffsetTable
...
class WindowedPanel:
    ...
    a one-shot :func:`saitenka.panel.render_panel` at the same offset (proven in
    ...
    def __init__(
        ...
        *,
        ...
        from saitenka.model import _DEFAULT_THEME
        from saitenka.render.layout_backend import DEFAULT_BACKEND
        from saitenka.render.window import LazyOffsets
    ...
    def _render_ahead_parallel(
        ...
        Dispatch is decided from the ACTUAL executor :func:`~saitenka.parallel.shared_executor` hands
