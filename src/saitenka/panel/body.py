...
This module-level worker boundary lets ``render/banded.py`` call :func:`render_body_block` on its
GIL-build process-pool path without ``panel_rows()`` closures in the way.
"""
...
if TYPE_CHECKING:
    ...
    from saitenka.model import LinkBox, ScanBox, Style
...
class BodyRenderArgs:
    ...
    without any of ``panel_rows()``'s closures — or the shared ``WindowedPanel`` state
...
class LaidOutBody:
    ...
    def geometry(self) -> tuple[list[ScanBox], list[LinkBox]]:
        ...
        from saitenka.render.document import document_geometry
...
def raster_body_window(
    ...
    from saitenka.render.document import render_layout_window
...
def render_body_block(
    ...
    """Render one def-body block: the SC-walk + document layout shared by ``panel_rows``'s deferred
    ...
    covering strip for a pathologically tall single block, not the whole thing (see
    ``Row.render_capped`` for why this bounds cold first paint to O(viewport))."""
    scan: list[ScanBox] = []
    links: list[LinkBox] = []
    clipped: list = []
    img = render_document(
