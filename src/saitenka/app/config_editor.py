...
* **Schema (SSOT):** the frozen :mod:`~saitenka.app.config` dataclasses. :func:`catalog` introspects them
  ...
  profiles build on.)
* **Prompts:** :mod:`~saitenka.app.prompt` (``confirm``/``select``/``text`` — questionary, isatty-gated,
  numbered fallback). This module only maps a field type onto the right primitive and coerces the reply.
* **Write:** :func:`~saitenka.app.init_wizard.write_config` (tomlkit round-trip — preserves comments and
