...
def _anki_call(action: str, **params):
    ...
    Raises from :data:`~saitenka.app.anki.ANKI_DOWN_ERRORS`; callers catch that and warn."""
...
def check_resync() -> Check:
    ...
    from saitenka.mpvio.discover import find_healthy_tool, find_tool
...
def check_profiles() -> list[Check]:
    ...
    from saitenka.app.profiles import profile_names
...
def check_anki(deck: str, model: str) -> Check:
    from saitenka.app.anki import resolve_anki
...
def check_plugin() -> Check:
    ...
    * a **bare** ``SAITENKA_BIN`` that a Finder/Dock-launched mpv can't resolve on its minimal PATH,
    ...
    from saitenka.app.plugin import LUA_NAME, all_scripts_dirs
...
def check_jimaku() -> Check:
    ...
    from saitenka.app.jimaku import resolve_jimaku_key
    ...
    if src == "env":
        ...
        from saitenka.app.jimaku import keychain_get
...
def _dictdb_seq_configured_but_unpopulated() -> bool:
    ...
    from saitenka.app.config import resolve_dictdb
    from saitenka.app.dictdb import DictionaryDb, db_path
    from saitenka.app.dictionary import _looks_like_jmdict
...
def check_subminer_conflict() -> Check:
    ...
    from saitenka.app.conflicts import subminer_installed, subminer_running
...
def check_perf() -> Check:
    """Live latency + memory snapshot (render, hover hit-test, RSS) from :mod:`saitenka.app.perf` — the
    ...
    from saitenka.app.perf import rss_mb, snapshot
...
def check_telemetry() -> Check:
    ...
    :func:`saitenka.otel_metrics.snapshot`); it reports config state + the on-disk CTF trace
...
def check_mpv_socket() -> Check:
    ...
    if sock:
        from saitenka.mpvio.ipc import is_windows_pipe_path, normalize_ipc_path
    ...
    if sys.platform == "win32":
        from saitenka.mpvio.ipc import MPVNET_DEFAULT_PIPE
