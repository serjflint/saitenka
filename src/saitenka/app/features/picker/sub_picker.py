...
and no mpv IPC ever runs off the reader thread. Modelled on the sidebar surface (click / scroll /
hover suppression) and help overlay lifecycle.
"""
...
def _rows(state: PickerState) -> list[SidebarRow]:
    ...
    mixed = len({language_base(candidate.language or "") for candidate in candidates}) > 1
