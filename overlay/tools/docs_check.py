...
_PKG = _OVERLAY / "src" / "overlay"  # overlay/src/overlay
...
def _doc_files() -> list[Path]:
    docs = [_REPO / "AGENTS.md", _OVERLAY / "ARCHITECTURE.md", _OVERLAY / "README.md"]
...
# pkg-qualified source file, e.g. `render/banded.py` — resolved under overlay/src/overlay. The
...
# tool/script file under overlay/, e.g. `tools/mutate/run.py`, `tools/semgrep/rules.yml`.
_TOOL_FILE = re.compile(r"(?<![\w/])tools/[A-Za-z0-9_./-]+\.(?:py|yml|toml)")
# explicit repo-relative overlay path with a known extension.
...
def _ref_failures(text: str, rel: str, tasks: set[str]) -> list[str]:
    ...
    return [
        ...
        *(
            f"{rel}: module `{m}` not found under overlay/src/overlay/"
            for m in modules
            if not (_PKG / m).is_file()
        ),
        *(f"{rel}: `{m}` not found under overlay/" for m in toolz if not (_OVERLAY / m).is_file()),
...
def _cfg(cls: str, attr: str) -> Callable[[], object]:
    def go() -> object:
        mod = importlib.import_module("overlay.app.config")
