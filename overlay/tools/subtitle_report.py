...
unzipped dirs (or a bare `trace.json`):
    ...
    uv run overlay/tools/subtitle_report.py ~/.local/share/saitenka/reports/saitenka-report-*.zip
    uv run overlay/tools/subtitle_report.py --json report-A.zip report-B.zip > seeds.json
...
imports the `overlay` package, so run that mode in the project env with `uv run python`:
    ...
    uv run python overlay/tools/subtitle_report.py --video EP.mkv --sub ~/Library/Caches/saitenka/subtitles

The report mode is stdlib-only (reuses the loaders from the sibling `trace_report.py`); reproduce lazily
imports `overlay.app.resync` so there is one alignment implementation, never a drifting copy.
"""
...
def reproduce(video: Path, sub: Path) -> dict:
    ...
    ``overlay.app.resync`` (one alignment implementation), so run it in the project env with
    ``uv run python``."""
    try:
        ...
        raise SystemExit(
            ...
            "  uv run python overlay/tools/subtitle_report.py --video EP.mkv --sub CACHE"
        ) from exc
    import tempfile
