...
def _repo(tmp_path: Path) -> Path:
    ...
    (tmp_path / "src/overlay/app/config.py").write_text("class Config: pass\n", encoding="utf-8")
    (tmp_path / "src/overlay/app/controller.py").write_text(
        "class Reader:\n    _state = 1\n", encoding="utf-8"
    )
    (tmp_path / "tests/test_shared.py").write_text(
        "from overlay.app.config import Config\n"
        "from overlay.app.controller import Reader\n\n"
        "def make_reader():\n    return Reader()\n\n"
        "def test_controller_state():\n    assert make_reader()._state == 1\n",
        encoding="utf-8",
    )
    return tmp_path
