...
:class:`~overlay.app.subselect.SubtitleCandidate` rows and runs the chosen one's ``download`` thunk
through the normal subtitle-fetch pipeline (:func:`overlay.app.subtitle_modes.start_fetch` →
:func:`~overlay.app.subtitle_modes.apply_fetch_results`), so track add / select / re-index come free
and no mpv IPC ever runs off the reader thread. Modelled on :mod:`overlay.app.sidebar` (click / scroll
/ hover-suppression surface) and :mod:`overlay.app.help_overlay` (open / close lifecycle).
"""
...
from overlay.app.subtitles import SidebarRow, render_picker
