...
already agrees on (:func:`overlay.app.continuity.episode_identity`) and the cue span, so the sidebar Mine
tab can list an episode's mined cards without Anki and round-trip a preview to the exact line.

Mirrors :class:`overlay.app.backlog.BacklogStore`: one SQLite connection owned by the Reader thread.
"""
...
def ensure_store(reader) -> MinedCardStore:
    ...
    the backlog). The single seam both the mine-time writer (:mod:`overlay.app.miner`) and the Mine-tab
    reader (:mod:`overlay.app.sidebar`) go through."""
