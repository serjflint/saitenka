...
def test_cycle_is_a_noop_with_a_single_profile():
    ...
    reader = _headless()  # profiles defaults to (DEFAULT_PROFILE,)
...
def test_pressing_the_key_cycles_the_reading_identity(monkeypatch):
    ...
    import overlay.app.subtitle_providers as prov
...
@pytest.mark.usefixtures("_restore_tokenizer_registry")
def test_cycle_clears_the_token_cache_so_stale_segmentation_cannot_leak():
    ...
    register_tokenizer("latin", lambda: _MinimalTokenizer("latin"))
    reader = _headless(profile=DEFAULT_PROFILE, profiles=[DEFAULT_PROFILE, _FR])
...
def test_cycle_reverts_atomically_when_the_new_tokenizer_is_unknown():
    ...
    reader = _headless(profile=DEFAULT_PROFILE, profiles=[DEFAULT_PROFILE, _BROKEN])
...
def test_swap_during_warm_drops_the_stale_language_entry():
...
def test_warm_under_the_new_generation_stores_cleanly_after_a_swap():
