...
_HOOKS = Path(__file__).resolve().parent.parent.parent / ".agents" / "hooks"
...
def test_map_surfaces_kinds_for_a_touched_subsystem():
    ...
    hits = tk.applicable(["overlay/src/overlay/app/render_cache.py"])
...
def test_map_unions_kinds_when_a_path_matches_several_subsystems():
    tk = _load("test_kinds", "test_kinds.py")
    names = {n for n, _ in tk.applicable(["overlay/src/overlay/app/controller.py"])}
...
def test_map_stays_quiet_for_docs_and_test_only_changes():
    ...
    tk = _load("test_kinds", "test_kinds.py")
    assert tk.applicable(["README.md", "overlay/tests/test_foo.py", "docs/index.md"]) == []


def test_signature_is_stable_and_set_based():
    tk = _load("test_kinds", "test_kinds.py")
    a = tk.applicable(["overlay/src/overlay/app/dictionary.py"])
