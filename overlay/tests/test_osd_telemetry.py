...
covers every current and future overlay. The recorded ``w``/``h`` are the ACTUAL uploaded pixels, so
...
def test_geometry_tracks_ui_scale(monkeypatch):
    ...
    from overlay.render.help import HelpEntry, build_document, render_page
