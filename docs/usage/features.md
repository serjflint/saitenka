...
- ++ctrl+shift+t++ retries the enabled provider chain (jimaku, then opt-in TsukiHime) for the
  current media without switching tracks. TsukiHime can surface text attachments imported from
  AniMeTosho, in whichever of the languages a release carries matches your profile's target.
