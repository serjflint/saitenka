...
The packaged `yomitan_oracle.mjs` resource loads Yomitan's real `DictionaryImporter`, `DictionaryDatabase`, and
`Translator` from an external checkout. No GPL implementation or fixture is copied into yomitanlite.
...
runner with Node. The packaged `upstream-lock.json` records the reviewed revisions of Yomitan, yomitan-api, and
anki_miner. Updating a revision is a deliberate oracle review, never an automatic fixture re-bless.
