...
| **Repo hooks** (`.agents/hooks/`, committed & agent-agnostic) | `.claude/settings.json`: `PreToolUse[Bash]` → `block-shell-search.py` + `block-bare-python.py`; `SessionStart` → `repowise-refresh.py` | Codex `hooks.json`: the same scripts on the equivalent events (PreToolUse for the two guards; a session-start event for the refresh) |
