...
|---|---|---|---|
| agreement · scale-invariance · cache-equivalence · config-commutativity · feature-toggle · sub-pixel · concurrency · input-equivalence | metamorphic/invariant | *Metamorphic families* ↓ | `tests/test_scale_boundary.py` etc. |
...
| stateful / model-based | other | *Other realized kinds* ↓ | `tests/test_tooltip_statemachine.py` |
...
| **Stateful / model-based** | a `RuleBasedStateMachine` drives arbitrary action sequences against a model; the combination oracle rides on it | `tests/test_tooltip_statemachine.py` | interacting stateful features whose *sequences* (not single acts) carry the bug |
