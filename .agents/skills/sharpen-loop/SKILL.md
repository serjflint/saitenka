...
5. Validate the author's response against `.agents/sharpen/contracts.json`, then run the objective gate
   directly. Pass its structured result to `uv run python tools/sharpen_policy.py gate --mode
   <efficacy|conformance> --gate-json '<json>'`; this shared result owns disposition. Off the mutation
   allowlist, an assertion replacement/removal must pass the exact-text preservation witness. Retry at most
   three times, reverting only the patch created by the failed attempt.
...
- A completed dry-run receipt may feed `assurance-pipeline` before feature work begins. Return preservation,
  anti-cheat, and efficacy evidence; the reverted edit is a coordinate, not package-tree content.
