...
| **Scale-boundary / crisp path at scale>1** (#167–174) | yes | `app/tooltip.py` — dead-config `scale=2.0` | **3 context-delta** (+ **2**) | coverage-only | **empirical**: `tests/test_scale_boundary.py` + `test_tooltip_statemachine.py` pass; the drift negative control mis-hits | **surfaced + closed** |
