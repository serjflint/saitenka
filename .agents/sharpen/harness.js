...
const cfg = args || {}
const CONTRACT_VERSION = 5 // mirrors contracts.json; the Workflow runtime cannot read local files
...
async function recordOutcome(state, prop, reviewResult, extraNote) {
  ...
  const axes = prop ? { before: base.before, decisions: prop.proposals.map(p => p.change), diff: prop.diff } : { terminal: extraNote ?? 'no edit' }
