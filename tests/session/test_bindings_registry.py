...
def test_binding_messages_and_handlers_correspond_exactly():
    ...
    handler_msgs = set(build_session(FakeIPC()).command_runtime.names())
...
def test_every_command_spec_is_routed_and_keeps_its_owner_and_gates():
    ...
    """
    commands = build_session(FakeIPC()).command_runtime
    actual = {
...
def test_press_runs_a_real_handler_through_the_event_loop(monkeypatch):
    ...
    r.command_runtime.install_input()
    assert not r.help_controller.state.open

    press(r, ipc, ReaderOptions().keys.help_key)

    assert r.help_controller.state.open  # the keypress drove the real handler to mutate real state
...
def test_mine_key_fires_its_handler_after_anki_loads_post_registration(monkeypatch):
    ...
    r.command_runtime.install_input()  # bound while the dep is down
    config = MineConfig()
