...
def test_every_global_saitenka_binding_registers_without_deps(make_session):
...
def test_requires_gated_bindings_still_register_when_the_dep_is_absent(make_session):
    ...
    ipc = FakeIPC()
    options = ReaderOptions()
    r = make_session(ipc, services=SessionServices(anki=None), options=options)  # no anki, no tts
    r.graph.commands.install_input()
    reg = keybind_registry(ipc)
...
def test_binding_messages_and_handlers_correspond_exactly(make_session):
    ...
    handler_msgs = set(make_session(FakeIPC()).graph.commands.names())
...
def test_every_command_spec_is_routed_and_keeps_its_owner_and_gates(make_session):
    ...
    """
    commands = make_session(FakeIPC()).graph.commands
    actual = {
...
def test_press_runs_a_real_handler_through_the_event_loop(monkeypatch, make_session):
...
def test_mine_key_fires_its_handler_after_anki_loads_post_registration(monkeypatch, make_session):
    ...
    ipc = FakeIPC()
    r = make_session(ipc, services=SessionServices(anki=None))
...
def test_pressing_an_unbound_key_raises_so_the_fake_cant_pass_silently(make_session):
