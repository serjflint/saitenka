...
def index_cues(draw, min_size=1, max_size=6):
    ...
    for _ in range(draw(st.integers(min_size, max_size))):
        ...
        out.append(Cue(t, t + dur, draw(jp)))
...
@example(cues=[Cue(0.0, 1.0, "あ"), Cue(2.0, 3.0, "い")])  # t==0 lower-incl, t==1/3 upper-excl
...
@example(cues=[Cue(0.0, 1.0, "あ")], current=0, delta=1, inside=True)  # next past last → -1
@example(cues=[Cue(0, 1, "あ"), Cue(2, 3, "い")], current=1, delta=-1, inside=False)  # gap prev
@given(
...
def test_target_spec(cues, current, delta, inside):
    n = len(cues)
    r = CueIndex(cues).target(current, delta, inside=inside)
...
@example(cues=[Cue(0.0, 1.0, "あ"), Cue(2.0, 3.0, "い")], a=0.5, b=2.5)  # spans a gap + two cues
@given(
