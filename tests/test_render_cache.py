...
def test_cold_show_paints_directly_from_cache_before_building(tmp_path, monkeypatch):
    ...
    r.turn.tooltip_controller.select(i)
    tok = r.turn.subtitle_presentation.cue.current.tokens[i]
    cap, w = r.turn.tooltip_controller.scale().cap, r.turn.tooltip_controller.scale().width
    key = r.turn.tooltip_controller.panel_key(
        tok,
        r.turn.tooltip_controller.inflected_surface(i),
        mined=r.turn.tooltip_controller.is_mined(tok),
    )
    ...
    sig, ck = (
        r.turn.tooltip_preparation.cache.signature(r.turn.tooltip_controller.preparation_inputs),
        content_key(key),
    )
    cache.put(sig, ck, cap, cap, cap, sentinel)
    assert r.turn.tooltip_preparation.cache.hydrate(
        r.turn.tooltip_controller.preparation_inputs, key
    )

    uploaded: list = []
    monkeypatch.setattr(
        r.turn.ov, "show_bgra", lambda view, *_a, **_k: uploaded.append(np.array(view))
    )
    r.turn.tooltip_controller.surface_state().panel_cache.clear()  # force cold
    r.turn.tooltip_controller.show_tooltip(i)
    ...
    assert isinstance(
        r.turn.tooltip_controller.surface_state().view.state, Panel
...
def _nested_reader(*, two_words: bool = False):
    ...
    r.turn.screen.osd = (
        3840,
        2160,
    )  # 4K → tip_scale.raster 2.0, so _blit_native takes the crisp path
    ...
    if two_words:
        r.turn.subtitle_presentation.cue.current.tokens.append(
            Token("読書", "読書", "どくしょ", "名詞", 2, 4)
        )
        r.turn.subtitle_presentation.cue.current.boxes.append(WordBox(1, 420, 300, 40, 40))
    ...
    def scroll_to_the_cold_tail() -> None:
        for _ in range(6):  # each notch moves further past anything the show warmed
            tooltip_panel.scroll_view(
                ...
                r.turn.tooltip_controller.surface_state().nest.view_h,
            )

    assert (
        assert_no_interactive_raster(
            r.turn.tooltip_controller.surface_state().nest, scroll_to_the_cold_tail
        )
        == 0
    )
    assert r.turn.tooltip_controller.surface_state().nest.scroll > 0, (
...
def test_the_raster_oracle_can_fire():
    ...
    panel = r.turn.tooltip_controller.surface_state().nest.state
...
def test_warm_nested_scroll_upgrades_to_crisp_with_no_interactive_raster():
    ...
    tooltip_panel.scroll_view(
        r.turn.tooltip_controller.tip_ports,
        r.turn.tooltip_controller.surface_state().nest,
        r.turn.tooltip_controller.surface_state().nest.view_h // 2,
    )  # soft-first + records a render-ahead
    assert r.turn.tooltip_controller.surface_state().nest.crisp_pending
    _render_submitter(r).finish_all()

    rasters = assert_no_interactive_raster(
        r.turn.tooltip_controller.surface_state().nest,
        lambda: tooltip_panel.apply_pending_crisp(
            r.turn.tooltip_controller.tip_ports, r.turn.tooltip_controller.surface_state().nest
        ),
    )
    assert rasters == 0
    assert (
        r.turn.tooltip_controller.surface_state().nest.crisp_miss == ""
        and not r.turn.tooltip_controller.surface_state().nest.crisp_pending
    )  # upgraded soft → crisp
...
def test_nested_scroll_requests_render_ahead_for_the_nested_view():
    ...
    nest = r.turn.tooltip_controller.surface_state().nest.state
    tooltip_panel.scroll_view(
        r.turn.tooltip_controller.tip_ports,
        r.turn.tooltip_controller.surface_state().nest,
        max(1, r.turn.tooltip_controller.surface_state().nest.view_h // 3),
    )
...
def _render_ahead_scales(r, view, monkeypatch) -> list[float]:
    ...
    tooltip_panel.scroll_view(
        r.turn.tooltip_controller.tip_ports, view, view.view_h
    )  # flick → records a render-ahead
    _render_submitter(r).finish_all()
    return seen
...
def test_render_ahead_warms_raw_bands_ahead_on_hidpi(monkeypatch):
    ...
    monkeypatch.setattr(r.turn.ov, "show_bgra", lambda *_a, **_k: None)
    r.turn.tooltip_controller.show_tooltip(0)
    scales = _render_ahead_scales(r, r.turn.tooltip_controller.surface_state().view, monkeypatch)
    assert r.turn.tooltip_controller.scale().raster > 1.0  # the fixture is 4K → native scale
    assert (
        r.turn.tooltip_controller.scale().raster in scales
    )  # native bands warmed ahead (unchanged)
...
def test_render_ahead_warms_raw_once_on_lodpi(monkeypatch):
    ...
    r = _nested_reader()
    r.turn.screen.osd = (1920, 1080)  # lo-dpi → tip_scale.raster 1.0
    r.turn.tooltip_controller.select(0)
    monkeypatch.setattr(r.turn.ov, "show_bgra", lambda *_a, **_k: None)
    r.turn.tooltip_controller.show_tooltip(0)
    scales = _render_ahead_scales(r, r.turn.tooltip_controller.surface_state().view, monkeypatch)
    assert r.turn.tooltip_controller.scale().raster == 1.0
...
def test_soft_nested_paint_upgrades_the_nested_view_not_the_base(monkeypatch):
    ...
    r.turn.tooltip_controller.select(0)
    r.turn.tooltip_controller.show_tooltip(0)  # base tooltip up, cold → its own crisp_pending
    assert r.turn.tooltip_controller.surface_state().view.crisp_pending
    tok = r.turn.subtitle_presentation.cue.current.tokens[
        1
    ]  # nested on a DIFFERENT word, so warming it can't warm the base
    nested_popup.open_nested(
        r.turn.tooltip_controller.tip_ports,
        r.turn.tooltip_controller.panel_ports,
        tok,
    ...
    assert (
        r.turn.tooltip_controller.surface_state().nest.crisp_pending
    )  # nested has its OWN pending flag…
    assert (
        r.turn.tooltip_controller.surface_state().view.crisp_pending
    )  # …and did NOT clobber the base's

    nest = r.turn.tooltip_controller.surface_state().nest.state
    assert nest is not None
    vh = min(r.turn.tooltip_controller.surface_state().nest.view_h, nest.full_height)
    y0 = max(
        0, min(r.turn.tooltip_controller.surface_state().nest.scroll, max(0, nest.full_height - vh))
    )
    nest.viewport(
        y0, vh, overscan=vh, scale=r.turn.tooltip_controller.scale().raster
    )  # worker warms ONLY the nested native

    uploads: list = []
    monkeypatch.setattr(r.turn.ov, "show_bgra", lambda _v, *_a, oid=None, **_k: uploads.append(oid))
    tooltip_panel.apply_pending_crisp(
        r.turn.tooltip_controller.tip_ports, r.turn.tooltip_controller.surface_state().view
    ...
    assert (
        not r.turn.tooltip_controller.surface_state().nest.crisp_pending
        and r.turn.tooltip_controller.surface_state().nest.crisp_miss == ""
    )  # nested is crisp now
    assert (
        r.turn.tooltip_controller.surface_state().view.crisp_pending
...
def test_preloop_demo_hover_is_ready_with_a_real_runtime_gateway(tmp_path, monkeypatch):
    ...
    r.turn.tooltip_controller.surface_state().panel_cache.clear()
    try:
        r.turn.tooltip_controller.prepare_hover_blocking(i)
        assert r.turn.tooltip_controller.surface_state().view.state is not None
        assert r.turn.tooltip_controller.surface_state().view.state is not None
    finally:
        r.close()
        gateway.close()
...
def _first_content(r):
    ...
    return (
        ...
        tok,
        r.turn.tooltip_controller.inflected_surface(i),
        r.turn.tooltip_controller.is_mined(tok),
    )
...
def test_worker_seed_head_hydrates_tier2_from_disk_no_raster(tmp_path, monkeypatch):
    ...
    cap = r.turn.tooltip_controller.scale().cap
    st = r.turn.tooltip_controller.panel_for(tok, inflected, min_h=cap, mined=mined)
    r.turn.tooltip_preparation.cache.precompose_head(
        r.turn.tooltip_controller.preparation_inputs,
        st,
        tok,
        inflected,
        mined=mined,
    ...
    r.turn.tooltip_controller.surface_state().panel_cache.clear()  # simulate the in-memory panel evicted
    fresh = r.turn.tooltip_controller.panel_for(tok, inflected, min_h=cap, mined=mined)
    assert fresh.windowed.first_view is None  # cold rebuild
    assert r.turn.tooltip_preparation.cache.worker_seed_head(
        r.turn.tooltip_controller.preparation_inputs,
        fresh,
        tok,
        inflected,
    ...
    assert r.turn.tooltip_preparation.cache.memory_entries == 1
    assert (
        r.turn.tooltip_preparation.cache.peek(
            r.turn.tooltip_controller.preparation_inputs,
            r.turn.tooltip_controller.panel_key(tok, inflected, mined=mined),
        )
        is not None
    )  # main hits RAM
...
def test_cold_miss_defers_showing_nothing_and_enqueues(tmp_path, monkeypatch):
    ...
    i, _tok, _inflected, _mined = _first_content(r)
    r.turn.tooltip_controller.surface_state().panel_cache.clear()  # ensure cold
    tooltip.show_tooltip(
        r.turn.tooltip_controller.tip_ports,
        r.turn.tooltip_controller.panel_ports,
        r.turn.tooltip_controller.hover_inputs,
        r.turn.tooltip_controller.show_actions,
        i,
    )
    assert r.turn.tooltip_controller.surface_state().view.state is None  # nothing shown — deferred
...
def test_engaged_render_composes_then_completion_shows_warm(tmp_path, monkeypatch):
    ...
    key = tooltip_panel.panel_key(
        ...
        inflected,
        mined=mined,
        phrase=r.turn.tooltip_controller.observation().metadata.terms,
    )
    ...
    assert r.turn.tooltip_controller.surface_state().view.state is None and submitter.calls
    r.turn.tooltip_controller.select(i)
    submitter.finish()
    assert cache.stats()[0] == 1 and r.turn.tooltip_preparation.cache.memory_entries == 1

    # Still hovering the same word → completion shows the now-warm tooltip.
    assert r.turn.tooltip_controller.surface_state().view.state is not None  # shown
    assert tuple(r.turn.tooltip_controller.surface_state().view.key) == tuple(key)
...
def test_engaged_render_failure_emits_a_terminal_tooltip_outcome(tmp_path, monkeypatch):
    ...
    _i, tok, inflected, mined = _first_content(r)
    key = tooltip_panel.panel_key(
        r.turn.tooltip_controller.panel_ports, tok, inflected, mined=mined
    )
    spans = []
    ...
    submitter = _enable_engaged(r)
    r.turn.tooltip_controller.surface_state().view.job_id = (
        r.turn.tooltip_controller.surface_state().jobs.begin("tooltip")
    )
    assert r.turn.tooltip_controller.request_engaged_tooltip(
        tooltip_engaged.HoverRequest(
            ...
            job_id=r.turn.tooltip_controller.surface_state().view.job_id,
        )
    )
...
def test_engaged_render_capability_change_does_not_strand_hover(tmp_path, monkeypatch):
    ...
    submitter = _enable_engaged(r)
    i, tok, inflected, mined = _first_content(r)
    old_key = tooltip_panel.panel_key(
        r.turn.tooltip_controller.panel_ports, tok, inflected, mined=mined
    )
    r.turn.tooltip_controller.surface_state().view.job_id = (
        r.turn.tooltip_controller.surface_state().jobs.begin("tooltip")
    ...
    assert r.turn._tts_capability is not None
    r.turn._tts_capability._publish(0, available=True)
    r.turn.tooltip_controller.select(i)
    monkeypatch.setattr(tooltip, "_paint_from_cache", lambda *_args: False)
    submitter.finish()

    assert r.turn.tooltip_controller.surface_state().view.state is not None or submitter.calls
    if submitter.calls:
...
def test_engaged_result_discarded_when_word_changed(tmp_path, monkeypatch):
    ...
    r.turn.tooltip_controller.surface_state().view.state = None  # not hovering that word anymore
    ...
    assert r.turn.tooltip_controller.surface_state().view.state is None  # nothing shown
...
def test_engaged_result_cannot_drive_a_new_hover_job(tmp_path, monkeypatch):
    ...
    r, _cache_obj = _tall_reader(tmp_path, monkeypatch)
    i, _tok, _inflected, _mined = _first_content(r)
    old_job = r.turn.tooltip_controller.surface_state().jobs.begin("tooltip")
    r.turn.tooltip_controller.select(i)
    r.turn.tooltip_controller.surface_state().view.job_id = (
        r.turn.tooltip_controller.surface_state().jobs.begin("tooltip")
    )
    ...
    assert r.turn.tooltip_controller.surface_state().view.state is None
    assert r.turn.tooltip_controller.surface_state().view.state is None
...
def test_nested_cold_miss_defers_and_enqueues_nested(tmp_path, monkeypatch):
    ...
    nested_popup.open_nested(
        ...
        tok,
        inflected,
        nested_popup.Anchor(5, 300, 20),
        defer=True,
    )
    assert r.turn.tooltip_controller.surface_state().nest.state is None  # nothing shown — deferred
...
def test_nested_no_worker_opens_synchronously(tmp_path, monkeypatch):
    ...
    assert r.turn.tooltip_controller.surface_state().nest.state is not None  # shown synchronously
    assert r.turn.tooltip_controller.surface_state().nest.state is not None
...
def test_engaged_nested_composes_warms_bands_without_disk(tmp_path, monkeypatch):
    ...
    st = r.turn.tooltip_controller.surface_state().panel_cache.get(
        key
    )  # panel warmed into the cache
    assert st is not None
    vh = min(
        st.full_height,
        r.turn.tooltip_controller.cap_for(r.turn.tooltip_controller.visual.nested_height_fraction),
    )
...
def test_engaged_nested_drain_reopens_warm(tmp_path, monkeypatch):
    ...
    while not submitter.calls and time.monotonic() < deadline:
        time.sleep(0.001)
    assert r.turn.tooltip_controller.surface_state().nest.state is None and submitter.calls
    submitter.finish()
    assert (
        r.turn.tooltip_controller.surface_state().nest.state is not None
    )  # nested popup now shown
...
def test_mined_generation_change_requeues_current_hover_metadata(tmp_path, monkeypatch):
    ...
    requests = []
    monkeypatch.setattr(r.turn.tooltip_controller, "request_interaction_metadata", requests.append)
    r.turn.tooltip_controller.select(index)
    r.turn.tooltip_controller.surface_state().view.job_id = (
        r.turn.tooltip_controller.surface_state().jobs.begin("tooltip")
    )
    ...
    original = requests[-1]
    r.turn.mining_controller.record_mined_expression("__newly-mined__")
    ...
    assert requests[-1].key.mined_generation == r.turn.mining_controller.index_snapshot().generation
    assert requests[-1].key.job_id == r.turn.tooltip_controller.surface_state().view.job_id
...
def test_engaged_nested_dropped_when_cursor_left(tmp_path, monkeypatch):
    ...
    r, _cache_obj = _tall_reader(tmp_path, monkeypatch)
    (
        r.turn.tooltip_controller.surface_state().view.state,
        r.turn.tooltip_controller.surface_state().view.rect,
    ) = None, None
    tooltip.apply_engaged_hover(
        r.turn.tooltip_controller.tip_ports,
        r.turn.tooltip_controller.panel_ports,
    ...
    assert r.turn.tooltip_controller.surface_state().nest.state is None
...
def _base_tip_up(r):
    _i, tok, inflected, mined = _first_content(r)
    r.turn.tooltip_controller.surface_state().view.state = r.turn.tooltip_controller.panel_for(
        tok, inflected, min_h=r.turn.tooltip_controller.scale().cap, mined=mined
    )
    return tok
...
def test_clicked_nav_defers_when_worker_running(tmp_path, monkeypatch):
    ...
    tooltip.navigate_tip(
        r.turn.tooltip_controller.tip_ports, r.turn.tooltip_controller.panel_ports, tok.surface
    )
    request = submitter.calls[-1]["request"].request
    assert isinstance(request, tooltip_engaged.NavigateRequest) and request.query == tok.surface
    assert (
        r.turn.tooltip_controller.observation().navigation_depth == 0
    )  # nothing pushed / swapped yet
...
def test_clicked_nav_no_worker_navigates_synchronously(tmp_path, monkeypatch):
    ...
    assert (
        r.turn.tooltip_controller.observation().navigation_depth == 1
...
def test_engaged_nav_composes_then_swaps_from_warm_bands(tmp_path, monkeypatch):
    ...
    submitter = _enable_engaged(r)
    tok = _base_tip_up(r)
    old = r.turn.tooltip_controller.surface_state().view.state
    tooltip.navigate_tip(
        r.turn.tooltip_controller.tip_ports, r.turn.tooltip_controller.panel_ports, tok.surface
    )  # defer
    submitter.finish()
    assert (
        ...
        and r.turn.tooltip_controller.surface_state().view.state is not old
    )  # navigated panel installed
    assert (
        r.turn.tooltip_controller.observation().navigation_depth == 1
    )  # previous view pushed for Esc/back
    assert r.turn.tooltip_controller.surface_state().view.key is None  # a navigated view is keyless
...
def test_rejected_new_generation_uses_its_own_sync_fallback(tmp_path, monkeypatch):
    ...
    assert r.turn.tooltip_controller.request_engaged_tooltip(
        tooltip_engaged.HoverRequest(
            tok, tok.surface, mined=False, key=("old",), cap=r.turn.tooltip_controller.scale().cap
        )
    )
    r.turn.tooltip_preparation.cancel()
    r.turn.tooltip_controller.cancel_current_work()
    assert r.turn.tooltip_controller.request_engaged_tooltip(
        tooltip_engaged.NavigateRequest(tok.surface, id(old))
    )
...
def test_engaged_nav_dropped_when_tooltip_changed(tmp_path, monkeypatch):
    ...
    j = next(
        k
        for k, t in enumerate(r.turn.subtitle_presentation.cue.current.tokens)
        if t.is_content and t.surface != tok.surface
    )
    r.turn.tooltip_controller.surface_state().view.state = r.turn.tooltip_controller.panel_for(
        r.turn.subtitle_presentation.cue.current.tokens[j],
        r.turn.tooltip_controller.inflected_surface(j),
        min_h=r.turn.tooltip_controller.scale().cap,
        mined=False,
    )
    call["on_finished"](
        EffectFinished(
...
def test_stale_engaged_nav_failure_skips_sync_rebuild(tmp_path, monkeypatch):
    ...
    rebuilt = []
    monkeypatch.setattr(
        r.turn.tooltip_controller, "navigated_panel", lambda query: rebuilt.append(query)
    )

    submitter.finish(outcome=EffectOutcome.FAILED, run=False)

    assert rebuilt == [] and r.turn.tooltip_controller.observation().navigation_depth == 0
...
def test_the_panel_cache_evicts_the_least_recently_used_entry_at_the_cap():
    ...
    for key in ("a", "b", "c"):
        cache.setdefault(key, f"panel-{key}")
    cache.setdefault("a", "panel-a")  # touch: 'a' is now newest

    cache.setdefault("d", "panel-d")

    assert "b" not in cache  # the least recently used went, not the oldest inserted
    assert {"a", "c", "d"} <= {"a", "c", "d"}
    assert all(k in cache for k in ("a", "c", "d")) and len(cache) == 3
...
def test_a_second_writer_for_the_same_key_loses_to_the_first():
    ...
    second = cache.setdefault("k", "loser")
