...
def test_chrome_scale_grows_with_a_hi_dpi_osd():
    r = _reader(1.2)
    r.screen.osd = (3024, 1898)  # 2x Retina fullscreen — the reported setup
    assert r.screen.chrome_scale() == 1.2 * (1898 / REF_H)  # tracks osd_h like the tooltip/subtitle
    assert r.screen.chrome_scale() > 2.0  # visibly larger than the flat 1.2 that drew it small
