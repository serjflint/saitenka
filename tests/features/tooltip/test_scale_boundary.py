...
def test_drawn_element_round_trips_through_the_one_panel(scale, monkeypatch):
    ...
    for b in scans:
        ...
        assert (
            tooltip_panel.scan_hit(r.tooltip_controller.surface_state(), r.tip_scale.raster, mx, my)
            == b
        )
    for lb in links:
        ...
        assert (
            tooltip_panel.link_hit_at(
                r.tooltip_controller.surface_state(), r.tip_scale.raster, mx, my, nested=False
            )
            == lb
        )
