...
def _reader(tip_scale: float, osd_h: int) -> SessionGraph:
    r = build_session(
        util.FakeIPC(),
    ...
    r.graph.screen.osd = (round(osd_h * 16 / 9), osd_h)
    return r.graph


def test_auto_scale_tracks_the_video_viewport_when_tip_scale_is_zero():
    assert _reader(0.0, REF_H).tooltip.scale().display == 1.0  # 1080p reference
    assert _reader(0.0, 2 * REF_H).tooltip.scale().display == 2.0  # 4K → native 2×
...
def test_positive_tip_scale_overrides_the_resolution_derived_scale():
    ...
    assert _reader(1.5, REF_H).tooltip.scale().display == 1.5  # larger than native on 1080p
    assert _reader(1.5, 2 * REF_H).tooltip.scale().display == 1.5  # smaller than native on 4K
...
def test_raster_scale_snaps_osd_jitter_to_a_bucket():
    ...
    assert _reader(0.0, 2161).tooltip.scale().raster == 2.0  # 2.0009 → 2.00
    assert _reader(0.0, 2159).tooltip.scale().raster == 2.0  # 1.9991 → 2.00
    assert _reader(0.0, 1621).tooltip.scale().raster == 1.5  # 1.5009 → 1.50
...
def test_the_bucket_applies_to_a_hand_set_scale_too():
    ...
    scale = _reader(1.53, REF_H).tooltip.scale()
