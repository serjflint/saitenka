"""Fail when mining target, index, transaction, or store ownership escapes its boundary."""

from __future__ import annotations

import ast
import sys
from dataclasses import dataclass
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
APP = ROOT / "src" / "saitenka" / "app"

_OWNER = "mining_controller.py"
_COMPOSITION = "session_controller.py"
_CONSTRUCTORS = {
    "MinedCardStore": {_OWNER},
    "MinedSet": {_OWNER},
    "MiningIndexState": {_OWNER},
    "MiningSpec": {_OWNER, _COMPOSITION, "reader_deps.py"},
    "MiningTarget": {_OWNER, _COMPOSITION, "reader_deps.py"},
    "MiningTransaction": {_OWNER},
}
_OWNER_MUTATORS = {
    "publish_prepared_target": {_OWNER, _COMPOSITION},
    "record_expression": {_OWNER},
    "select_spec": {_COMPOSITION},
}
_LEGACY_FIELDS = {"anki", "mine_cfg", "mined_seed", "mined_store"}


@dataclass(frozen=True, slots=True)
class Finding:
    path: Path
    line: int
    rule: str
    detail: str


def _call_name(node: ast.Call) -> str | None:
    function = node.func
    if isinstance(function, ast.Name):
        return function.id
    if isinstance(function, ast.Attribute):
        return function.attr
    return None


def _assigned_attributes(node: ast.AST) -> list[ast.Attribute]:
    if isinstance(node, ast.Attribute):
        return [node]
    if isinstance(node, ast.Starred):
        return _assigned_attributes(node.value)
    if isinstance(node, ast.List | ast.Tuple):
        return [attribute for item in node.elts for attribute in _assigned_attributes(item)]
    return []


def inspect_source(source: str, path: Path) -> list[Finding]:
    tree = ast.parse(source, filename=str(path))
    filename = path.name
    findings: list[Finding] = []

    for node in ast.walk(tree):
        targets: list[ast.expr] = []
        if isinstance(node, ast.Assign):
            targets = node.targets
        elif isinstance(node, ast.AnnAssign | ast.AugAssign | ast.NamedExpr):
            targets = [node.target]
        for target in targets:
            findings.extend(
                Finding(path, attribute.lineno, "legacy-owner-field", attribute.attr)
                for attribute in _assigned_attributes(target)
                if attribute.attr in _LEGACY_FIELDS
            )

        if isinstance(node, ast.Call):
            name = _call_name(node)
            allowed = _CONSTRUCTORS.get(name or "")
            if allowed is not None and filename not in allowed:
                findings.append(Finding(path, node.lineno, "owned-constructor", name or ""))
            mutator_sites = _OWNER_MUTATORS.get(name or "")
            if mutator_sites is not None and filename not in mutator_sites:
                findings.append(Finding(path, node.lineno, "owner-mutator", name or ""))
            function = node.func
            if (
                isinstance(function, ast.Attribute)
                and function.attr in {"add", "update", "replace"}
                and isinstance(function.value, ast.Attribute)
                and function.value.attr == "mined"
            ):
                findings.append(Finding(path, node.lineno, "legacy-mined-state", function.attr))

    return sorted(findings, key=lambda finding: (str(finding.path), finding.line, finding.rule))


def inspect_tree(root: Path = APP) -> list[Finding]:
    findings: list[Finding] = []
    for path in sorted(root.glob("*.py")):
        findings.extend(inspect_source(path.read_text(encoding="utf-8"), path))
    return findings


def main() -> int:
    findings = inspect_tree()
    for finding in findings:
        relative = finding.path.relative_to(ROOT)
        print(f"{relative}:{finding.line}: {finding.rule}: {finding.detail}")
    if findings:
        print(f"mining-ownership: {len(findings)} violation(s)", file=sys.stderr)
        return 1
    print("mining-ownership: one writer for target, index, transaction, and store")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
