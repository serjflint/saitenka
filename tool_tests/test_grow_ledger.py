...
# --- target_sha termination properties -----------------------------------------------------------


def test_target_sha_is_stable_under_unrelated_line_drift():
    assert gl.target_sha(MODULE_V1, "crisp") == gl.target_sha(MODULE_V2, "crisp")


def test_target_sha_changes_when_the_target_symbol_changes():
...
def test_target_sha_reopens_on_a_decorator_swap():
    before = "class D:\n    @property\n    def v(self):\n        return 1\n"
    after = "class D:\n    @cached_property\n    def v(self):\n        return 1\n"
    assert gl.target_sha(before, "D.v") != gl.target_sha(after, "D.v")


def test_target_sha_reopens_on_a_redefinition_body_change():
    before = "def f():\n    return 1\n\n\ndef f():\n    return 2\n"
    after = "def f():\n    return 1\n\n\ndef f():\n    return 3\n"  # only the SECOND f changes
    assert gl.target_sha(before, "f") != gl.target_sha(after, "f")


def test_target_sha_ignores_a_comment_only_change():
...
def test_status_is_filed_when_a_product_issue_owns_the_unchanged_gap(tmp_path):
    root = _repo(tmp_path)
    rec = _rec(state="open", filed=["#201"])
    ledger = _ledger(root, [MANIFEST, rec])

    assert ledger.status(rec["gap_id"], root) == gl.FILED
...
def test_status_cli_reports_a_closed_coordinate(monkeypatch, tmp_path, capsys):
    root = _repo(tmp_path)
